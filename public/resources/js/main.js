...
// Fetch all tasks from API and add the to DOM list
getTask( (tasks) => {
  tasks.forEach(item => {
...
function removeItem() {
  ...
  req.open("POST", "/task/" + taskId + "/remove"); // initializes a newly-created request
  req.setRequestHeader('Content-Type', 'application/json');
  req.send(); // sends the data
...
const sendTasktoApi = (item, callback) => {
  ...
  let req = new XMLHttpRequest(); // built-in XMLHttpRequest Used to request data from a server 
  req.open("POST", "/task/add"); // initializes a newly-created request
  req.setRequestHeader('Content-Type', 'application/json');
  req.send(JSON.stringify({ item: item })); // sends the data
